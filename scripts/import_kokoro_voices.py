...
def import_one(lang,voice,out,source_dirs,force,retries):
    ...
    if target.exists() and not force:
        a=np.load(target,mmap_mode='r');return {'voice':voice,'path':str(target),'shape':list(a.shape),'status':'existing'}
    ...
    if local:
        ...
        for repo in REPOS[lang]:
            try:
                url=f'https://huggingface.co/{repo}/resolve/main/voices/{voice}.pt?download=true';a=tensor_from_pt(download(url,retries),voice);status='downloaded';source=url;break
            except Exception as e:
                last=e;print(f'  source {repo} failed for {voice}: {e}',file=sys.stderr)
        else:raise last
...
def main():
    ...
    langs=['en','zh'] if a.language=='all' else [a.language];a.output_dir.mkdir(parents=True,exist_ok=True);mp=a.output_dir/'manifest.json';manifest={'source':'https://huggingface.co/hexgrad','format':'numpy float32, final dimension 256','languages':{}}
    if mp.exists():
        try:manifest['languages'].update(json.loads(mp.read_text()).get('languages',{}))
        except (OSError,ValueError,TypeError):pass
    filt=set(a.voice or [])
    for lang in langs:
        names=[n for n in VOICE_FALLBACK[lang] if not filt or n in filt];names=names[:a.limit] if a.limit is not None else names;out=a.output_dir/lang;out.mkdir(parents=True,exist_ok=True);ok=[];failed=[];print(f'[{lang}] attempting {len(names)} voices from {MODELS[lang]}')
        for i,v in enumerate(names,1):
            try:r=import_one(lang,v,out,a.source_dir,a.force,max(0,a.retries));ok.append(r);print(f"  {i}/{len(names)} {v}: {r['status']}")
            except Exception as e:
                failed.append({'voice':v,'error':str(e)});print(f'  {i}/{len(names)} {v}: FAILED ({e})',file=sys.stderr)
                if not a.keep_going:break
        manifest['languages'][lang]={'model':MODELS[lang],'attempted':len(names),'count':len(ok),'voices':ok,'failed':failed}
        if failed and not a.keep_going:mp.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n');return 1
    mp.write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n');return 1 if any(manifest['languages'][l]['failed'] for l in langs) else 0
