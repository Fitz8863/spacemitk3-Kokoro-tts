...
std::string KokoroZhBackend::getConvFallbackFilter() const {
    // The official SpacemiT v1.1 graph contains rank-3 Resize nodes.
    // Keep these nodes on the ORT CPU fallback on the current K3 runtime;
    // otherwise their output time dimension can be doubled and the later
    // residual Add/decoder nodes fail shape broadcasting.
    return "/decoder/generator/conv_post/Conv;"
            ...
            "/decoder/decode.3/upsample/Resize";
}
